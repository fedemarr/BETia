...
export default function ValueBetCard({ bet }: Props) {
  ...
  const ourProb = bet?.ourProbability ?? 0
  const implied = bet?.impliedProbability ?? 0
  const odds    = bet?.marketOdds ?? 0

  const edgePct    = (edge * 100).toFixed(1)
  const evPct      = (ev * 100).toFixed(1)
  const ourProbPct = (ourProb * 100).toFixed(1)
  const impliedPct = (implied * 100).toFixed(1)
  const isStrong   = edge > 0.10

  return (
    <div className={clsx(
      'rounded-xl border p-4 space-y-3 transition-all',
      isStrong ? 'border-amber-500/60 bg-amber-500/10' : 'border-green-500/40 bg-green-500/8'
    )}>
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={clsx(
          ...
          isStrong ? 'bg-amber-500 text-black' : 'bg-green-500 text-black'
        )}>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
        <div>
          ...
          <p className="font-bold text-slate-100">{odds > 0 ? odds.toFixed(2) : '—'}</p>
        </div>
        <div>
